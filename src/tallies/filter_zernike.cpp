...
void ZernikeFilter::from_xml(pugi::xml_node node)
{
...
void ZernikeFilter::to_statepoint(hid_t filter_group) const
{
...
void ZernikeFilter::set_order(int order)
{
  if (order < 0) {
...
std::string ZernikeRadialFilter::text_label(int bin) const
{
  return "Zernike expansion, Z" + std::to_string(2 * bin) + ",0";
}

void ZernikeRadialFilter::set_order(int order)
{
...
std::pair<int, ZernikeFilter*> check_zernike_filter(int32_t index)
{
...
extern "C" int openmc_zernike_filter_get_order(int32_t index, int* order)
{
  // Check the filter.
...
extern "C" int openmc_zernike_filter_get_params(
...
extern "C" int openmc_zernike_filter_set_order(int32_t index, int order)
{
  // Check the filter.
  auto check_result = check_zernike_filter(index);
  auto err = check_result.first;
...
extern "C" int openmc_zernike_filter_set_params(
