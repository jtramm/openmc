...
void thick_target_bremsstrahlung(Particle& p, double* E_lost)
{
  ...
  double y = openmc::exp(y_l + (y_r - y_l) * f);
  ...
  *E_lost = 0.0;
  ...
  if (prn(p.current_seed()) <= f || j == 0) {
    ...
    double a = openmc::log(p_r / p_l) / (e_r - e_l) + 1.0;
    c_max = c_l + openmc::exp(e_l) * p_l / a * (openmc::exp(a * (e - e_l)) - 1.0);
  } else {
    i_e = j;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    double a = openmc::log(p_r / p_l) / (w_r - w_l) + 1.0;
    double w = openmc::exp(w_l) *
               openmc::pow(a * (c - c_l) / (openmc::exp(w_l) * p_l) + 1.0, 1.0 / a);
