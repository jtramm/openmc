// Fast-path arctangent function for binary64 values.
// Derived from the CORE-MATH project (MIT License).
// Original author: Alexei Sibidanov.
// https://core-math.gitlabpages.inria.fr/
//
// Accurate/refine path removed; only the fast path is retained.
...
double cr_atan(double x){
  ...
  if (at < 0x3f7b21c475e6362aull) {
    // |x| < 0x1.b21c475e6362ap-8
    if(at == 0) return x; // atan(+/-0) = +/-0
    ...
    if (at<(u64)0x3e40000000000000ull) { // |x| < 0x1p-27
      double res = std::fma(-0x1p-54, x, x);
      return res;
    }
  ...
  double h, ah, al;
  if(at>0x4062ded8e34a9035ull) {
    // |x| > 0x1.2ded8e34a9035p+7
    ah = std::copysign(0x1.921fb54442d18p+0, x);
    al = std::copysign(0x1.1a62633145c07p-54, x);
    if (at >= 0x434d02967c31cdb5ull) {
      // |x| >= 0x1.d02967c31cdb5p+53
      if (at > ((u64)0x7ff<<52)) return x + x; // NaN
      return ah + al;
    }
    h = -1.0/x;
  } else {
    ...
    double ta = std::copysign(1.0, x)*A[i][0], id = std::copysign(1.0, x)*(double)i;
    al = std::copysign(1.0, x)*A[i][1] + 0x1.8469898cc517p-55*id;
    h = (x - ta)/(1 + x*ta);
