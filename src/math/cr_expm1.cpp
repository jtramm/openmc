// Fast-path expm1 (e^x - 1) function for binary64 values.
...
// for -32 <= i <= 31, tz[i+32] is a double-double approximation of 2^(i/128) - 1
...
double cr_expm1(double x) {
  ...
  if (aix < 0x3fd0000000000000ull) {
    // Small |x| < 0.25: use polynomial on reduced argument
    if (aix < 0x3ca0000000000000ull) {
      if (!aix) return x;
      double res = std::fma(0x1p-54, std::fabs(x), x);
      return res;
    }
    double sx = 0x1p7 * x, fx = coremath_roundeven(sx), z = sx - fx, z2 = z * z;
    i64 i = fx;
    ...
    // Fast path only: return best approximation directly
    return fh + fl;
  } else {
    // Large |x| >= 0.25: range reduction and table lookup
    if (aix >= 0x40862e42fefa39f0ull) {
    ...
    double t = coremath_roundeven(x * s);
