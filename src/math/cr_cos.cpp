// Fast-path cosine function for binary64 values.
...
/* Convert two 64-bit integers (c1 high, c0 low) representing a 128-bit
   fractional value into a double-double h+l. */
static void
...
double cr_cos(double x)
{
  ...
  /* For |x| <= 0x1.6a09e667f3bccp-27, cos(x) rounds to 1 (to nearest). */
  t.u &= 0x7fffffffffffffff;
  if (t.u <= 0x3e46a09e667f3bcc)
    // |x| <= 0x1.6a09e667f3bccp-27
    return std::fma(t.f, -0x1p-28, 1.0);

  double h, l;
  cos_fast(&h, &l, t.f);
  /* In the original code, if fast path rounding test failed, the accurate
     path would be called. We return the fast result directly. */
  return h + l;
}
