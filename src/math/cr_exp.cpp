// Fast-path exponential function for binary64 values.
...
double cr_exp(double x) {
  ...
  if (aix <= 0x3c90000000000000ull)
    return 1.0 + x;

  if (aix >= 0x40862e42fefa39f0ull) { // |x| >= 0x1.62e42fefa39fp+9
    ...
    if (aix >= 0x40874910d52d3052ull) { // x <= -0x1.74910d52d3052p+9
      return 0x1.8p-1022 * 0x1p-55;    // underflow
    }
  }
