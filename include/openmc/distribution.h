...
public:
  ...
  PowerLaw(double a, double b, double n)
    : offset_ {openmc::pow(a, n + 1)}, span_ {openmc::pow(b, n + 1) - offset_},
      ninv_ {1 / (n + 1)} {};
  ...
  double a() const { return openmc::pow(offset_, ninv_); }
  double b() const { return openmc::pow(offset_ + span_, ninv_); }
